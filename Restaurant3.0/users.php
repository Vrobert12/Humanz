                        ...
                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="dropdownMenuLink">
                            <li><?php
                                if (isset($_SESSION['email']) && isset($_SESSION['name'])) { //A session segitsegevel megadjuk az adatok ertekeit
                                    ...
                                    echo "<a class=\"dropdown-item\" href=\"index.php?\"><i class=\"bi bi-arrow-return-left\"></i> Back to Main</a>";;
                                }
 ...
 //A session segitsegevel megadjuk az adatok ertekeit
    ...
    if (isset($_POST['searchAction'])) {
        if ($_POST['searchAction'] == 'search') {
            $data = "SELECT * FROM user where regisztraloEmail= '" . $_POST['searchMail'] . "'";
            users($data);
        } else {
...
<label>Verifikáció: ";
                ...
                if ($row['verify'] != 1) {
                    echo "No</label><br><label>Verifikciós kód: " . $row['verification_code'] . "</label><br><label>Virefikáció érvényessége: 
" . $row['verification_time'] . " </label><br>";
                } else {
                    echo "Yes</label><br><label>Verifikciós kód: NONE</label><br><label> Virefikáció érvényessége: NONE</label><br>";
                }
                if ($row['banned'] == 0) {
                    echo "<label>Tiltás: Nem</label><br><label> Tiltás ideje: NONE</label><br>";
                } else {
                    echo "<label>Tiltás: Igen</label><br><label> Tiltás ideje: " . $row['banned_time'] . "</label><br>";
                }
