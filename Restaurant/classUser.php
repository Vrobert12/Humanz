...
class User
{
    ...
    public function userString()
    {
        ...
        if($this->mail){
            $sql = mysqli_prepare($conn, "SELECT * FROM user WHERE privilage = ? AND userMail LIKE ?");
            $mailLike = "%" . $this->mail . "%"; // Prepare the mail variable with wildcards
            $sql->bind_param("ss", $this->rank, $mailLike);
        }
else{
